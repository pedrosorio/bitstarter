...
var gradermain = function(htmlstring, checksfile) {
...
if(require.main == module) {
	...
	.option('-u, --url <html_url>', 'Url to index.html')
        .parse(process.argv);
    if (program.url) {
	response2console = buildfn(program.checks);
	rest.get(program.url).on('complete',  response2console);
    }
    else {
	gradermain(fs.readFileSync(program.file), program.checks);
    }
} else {
